...
class PosCatalogFormNameWidget extends StatefulWidget {
  const PosCatalogFormNameWidget({Key? key}) : super(key: key);

  @override
  State<PosCatalogFormNameWidget> createState() =>
      _PosCatalogFormNameWidgetState();
}

class _PosCatalogFormNameWidgetState extends State<PosCatalogFormNameWidget> {
  late bool _initial;
      ...
      child: BlocBuilder<CatalogFormCreateCubit, CatalogFormCreateState>(
        ...
        if (p.initial != c.initial) {
          ...
          c.status.maybeWhen(
              ...
              orElse: () => null);
          return false;
        } else if (p.createCatalogItem.name != c.createCatalogItem.name) {
          if (_initial == true) _initial = false;
          return true;
        }
        return false;
        ...
        return Container(
          ...
          child: TextFormField(
              ...
              onChanged: (v) => BlocProvider.of<CatalogFormCreateCubit>(context)
                  .onCreateCatalogItemNameChanged(v)),
        );
      }),
    );
  }
