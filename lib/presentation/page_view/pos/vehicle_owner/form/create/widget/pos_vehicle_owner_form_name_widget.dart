...
class PosVehicleOwnerFormNameWidget extends StatefulWidget {
  const PosVehicleOwnerFormNameWidget({Key? key}) : super(key: key);

  @override
  State<PosVehicleOwnerFormNameWidget> createState() =>
      _PosVehicleOwnerFormNameWidgetState();
}

class _PosVehicleOwnerFormNameWidgetState
    extends State<PosVehicleOwnerFormNameWidget> {
  late bool _initial;
      ...
      child:
        ...
        if (p.initial != c.initial) {
          ...
          c.status.maybeWhen(
              ...
              orElse: () => null);
          return false;
        } else if (p.createVehicleOwner.name != c.createVehicleOwner.name) {
          if (_initial == true) _initial = false;
          return true;
        }
        return false;
          ...
          child: TextFormField(
              ...
              decoration: InputDecoration(
                errorText: _initial == false
                    ? state.createVehicleOwner.name.value.fold(
                        (l) => l.maybeWhen(
              ...
              onChanged: (v) =>
                  BlocProvider.of<VehicleOwnerFormCreateCubit>(context)
                      .onCreateVehicleOwnerNameChanged(v)),
        );
      }),
    );
  }
